...
def build_model(args, model_type):
    ...
    config.context_representation = args.context_representation   # 上下文decoder？　　论文中说ｐｒｏｍｐｔ扔进ｄｅｃｏｄｅｒ？
    ...
    for event_type, prompt in prompts.items():
        ...
        for token in token_list:
            if token.startswith('<') and token.endswith('>') and token not in new_token_list: #判断是否有其它特殊标志。 类似 <t>  </t> special tokens
                new_token_list.append(token)
    tokenizer.add_tokens(new_token_list)    #  bert_tokenizer.add_special_tokens({"additional_special_tokens": ADDITIONAL_SPECIAL_TOKENS}),
