...
class BaseTrainer:
    ...
    def train_one_step(self):
        self.model.train()
        try:
            batch = next(self.data_iterator)#获取一个batch数据，16个不同的变量，每个长度，即
        except StopIteration:
            self.data_iterator = iter(self.data_loader)
            batch = next(self.data_iterator)

        inputs = self.convert_batch_to_inputs(batch) #将批数据转换为dict形式，方便输入模型中。
        loss, _= self.model(**inputs)

        if self.cfg.gradient_accumulation_steps > 1: #如果这个大于1, 则要除以它。梯度累加更新
            loss = loss / self.cfg.gradient_accumulation_steps  #让loss 按积累步缩小倍数。让 gradient_accumulation_steps 次才达到一次的更新量
        loss.backward()
        ...
        if (self.metric['global_steps']+1)%self.cfg.gradient_accumulation_steps==0: #每 gradient_accumulation_steps更新下，就是说n步才更新损失。 global_steps : 总更新步数计数器。
            self.optimizer.step()
            self.scheduler.step()
