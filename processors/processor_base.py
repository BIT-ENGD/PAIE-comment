...
class DSET_processor:
    ...
    def _create_example_ace(self, lines):
        ...
        for line in lines:
            if not line['event']:
                continue
            events = line['event']   #只处理有事件的条目，其它忽略
            offset = line['s_start']  # start offset
            full_text = copy.deepcopy(line['sentence']) #copy.copy 也正常，只需要浅拷贝
            text = line['sentence']
            for event_idx, event in enumerate(events):
                event_type = event[0][1]
                event_trigger = dict()
                start = event[0][0] - offset; end = start+1 # start ,end offset 
                event_trigger['start'] = start; event_trigger['end'] = end
                event_trigger['text'] = " ".join(text[start:end])  ## start /end 为本sentence 中的偏移
                event_trigger['offset'] = offset  # 在全部数据中的偏移，原始数据中的偏移都是全数据中的，需要换算为本sentence中的偏移，以方便取数据。偏移是以词为元素的列表的位置。

                event_args = list()
                for arg_info in event[1:]:  # 取后面的roles 
                    arg = dict()
                    start = arg_info[0]-offset; end = arg_info[1]-offset+1  # 获取role 名字在本sentence中的偏移
                    role = arg_info[2]
                    arg['start'] = start; arg['end'] = end  # 设置角色在本sentence中的偏移
                    arg['role'] = role; arg['text'] = " ".join(text[start:end])
                    event_args.append(arg)  #得到某个角色名及角色的具体内容。

                examples.append(Event(event_idx, None, text, event_type, event_trigger, event_args, full_text))  # event_indx  赋给doc_id, sent_id为none , text/full_text可能有重复？
    ...
    def generate_dataloader(self, set_type):
        ...
        examples = self.create_example(file_path)  # 载入  train.json ， keep_ratio 表示 使用训练样本的比例，默认全用。
        if set_type=='train' and self.args.keep_ratio<1.0:
            sample_num = int(len(examples)*self.args.keep_ratio)
            examples = sample(examples, sample_num)  # 从弄表中随机抽取 sample_num个元素，就是随机抽样啦。
