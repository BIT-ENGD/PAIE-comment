...
def hungarian_matcher(predicted_spans, target_spans):
    ...
    indices = linear_sum_assignment(cost_spans.squeeze(0))   # 线性和分配问题
...
def get_best_span(start_logit, end_logit, old_tok_to_new_tok_index, max_span_length): #找最好跨度（准确度优先，可能会放弃一些潜在的匹配）
    # time consuming
    best_score = start_logit[0] + end_logit[0]
    best_answer_span = (0, 0)
    context_length = len(old_tok_to_new_tok_index) #对照表长度
...
def get_best_span_simple(start_logit, end_logit):  #这是简化匹配，找回值 最大的位置作为index.
    # simple constraint version
    _, s_idx = torch.max(start_logit, dim=0)
    _, e_idx = torch.max(end_logit[s_idx:], dim=0) # 要从start位置之后再找，总不能找前面的吧？
